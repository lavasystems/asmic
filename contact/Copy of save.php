        ...
        <table width="150" border="0">
          <tr> 
            <td align="center"> <input name="fullname" type="hidden" value="<? echo $tempname; ?>"> 
              ...
              <input name="icnum" type="hidden" value="<? echo $tempic; ?>"> <input name="email" type="hidden" value="<? echo $tempemail; ?>"> 
...
if ($mode == 'new')
{
	...
	for ( $x = 0; $x <= $_POST['addnum']; $x++ ) 
	{
		...
		//$address_primary = "address_primary_0";
		...
		if (empty($address_type) && empty($address_line1) && empty($address_line2) && empty($address_city) && empty($address_state) && empty($address_zip) && empty($address_phone1) && empty($address_phone2)) 
		{
			...
			{
				//echo $buat = "DELETE FROM " . TABLE_ADDRESS . " WHERE refid = $address_refid LIMIT 1";
				runQuery("DELETE FROM " . TABLE_ADDRESS . " WHERE refid = $address_refid LIMIT 1");
			}
		}
		else 
		{
			//echo $buat = "REPLACE INTO " . TABLE_ADDRESS . " VALUES ('$address_refid', $id, '$address_line1', '$address_city', '$address_state', '$address_zip', '$address_country', '$address_phone1', '$address_phone2')";
			runQuery("REPLACE INTO " . TABLE_ADDRESS . " VALUES ('$address_refid', $id, '$address_line1', '$address_city', '$address_state', '$address_zip', '$address_country', '$address_phone1', '$address_phone2')");
			if (empty($address_refid)) 
			{
...
if ($mode == 'delete') 
{
	...
	//mysql_query("UPDATE " . TABLE_EMAIL . " SET email = '$email' WHERE id = '$temp_id' LIMIT 1 ;", $db_link);
	//runQuery("DELETE FROM " . TABLE_ADDRESS . " WHERE id=$id");
	//runQuery("DELETE FROM " . TABLE_EMAIL . " WHERE id=$id");
	//runQuery("DELETE FROM " . TABLE_CONTRIBUTION . " WHERE id=$id");
	//runQuery("DELETE FROM library_member WHERE user_id=$id");
}
...
if ($_POST['groups']) 
{
	...
	while (list ($x_key, $x_gid) = each ($_POST['groups'])) 
	{
		...
		runQuery($groupsql);
		$audit_trail->writeLog($_SESSION['username'], "contact", "Add group $x_gid for $temp_name");			
	}
}
else
{
...
if ($_POST['areas']) 
{
	...
	while (list ($x_key, $x_gid) = each ($_POST['areas'])) 
	{
		$audit_trail->writeLog($_SESSION['username'], "contact", "Add new area of expertise: $x_gid to $temp_name");
...
{	
	...
	if (!empty($email))
	{
		...
		//{	
			...
			{
				mysql_query("INSERT INTO " . TABLE_EMAIL . " VALUES ($id, '$email')", $db_link);
				$audit_trail->writeLog($_SESSION['username'], "contact", "Insert email for $temp_name");			
			}
		//}
		//else
		//{
			//reportScriptError("Invalid e-mail address");
		//}
	}
}
...
{	
	...
	if (empty($temp_ref_id))
	{
		...
		$con_sql = "INSERT INTO 
					...
					con_zip, con_country, con_phone1, con_phone2, con_resume1, con_resume2) 
					VALUES ('$id', '$con_line1', '$con_city', '$con_state', '$con_zip', '$con_country', '$con_phone1', 
					'$con_phone2', '". $clsFileUpload1->getNewFileName() ."', 
...
if ($mode == 'edit')
{
	...
	$t_audit = mysql_fetch_array($audit);
	$temp_name = $t_audit['fullname'];
	$audit_trail->writeLog($_SESSION['username'], "contact", "Update confidentials for $temp_name");			
}
...
if (($mode == 'edit') || ($mode == 'new'))
{
	...
	$my_uploader->max_image_size(140, 140);
	// UPLOAD the file
	$my_uploader->upload("userfile2", "", ".jpg");
	...
	if ($my_uploader->error) 
	{
		...
		if ($memberstatus == 1)
		{
			/*$sqllibrary = "INSERT INTO library_member 
							...
							VALUES ($id, '$temp_card', '". date("Y-m-d")."', '". date("Y-m-d H:i:s")."')";
			mysql_query($sqllibrary, $db_link);*/
