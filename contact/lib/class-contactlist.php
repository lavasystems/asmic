...
class ContactList 
{
	...
	function group_name() 
	{
		...
		if ($this->group_id == 0) $this->group_name = "All Entries";
		// group_id = 1 --> "Unassigned Entries"
		elseif ($this->group_id == 1) $this->group_name = "Unassigned Entries";
		// group_id = 2 --> "Temporaries Entries"
		elseif ($this->group_id == 2)
		{
			...
			$this->group_name = "Temporary Entries"; // "Hidden Entries"
		}
	...
	function retrieve($sort) 
	{
		...
		$this->sql = "SELECT 
						...
						city, state, 
						zip, phone1, 
						phone2, phone3, country, 
						whoAdded, delflag, hidflag
		...
		if ($this->group_id == 0) 
		{
			$sql_group = " LEFT JOIN " . TABLE_ADDRESS . " AS address 
							ON contact.id=address.id 
							AND contact.primaryAddress=address.refid
						WHERE contact.hidden != 1 AND contact.delflag != 1 AND contact.hidflag != 1";
    	}
	    ...
	    elseif ($this->group_id == 1) 
		{
			/*$sql_group = " LEFT JOIN " . TABLE_ADDRESS . " AS address 
							...
							ON groups.id=contact.id
						WHERE groups.id IS NULL AND contact.hidden != 1";*/
			$sql_group = ", " . TABLE_GROUPS . " AS groups
						LEFT JOIN ". TABLE_ADDRESS ." AS address 
						ON contact.id=address.id 
						AND contact.primaryAddress=address.refid
						WHERE contact.id=groups.id AND groups.groupid = 1 AND contact.hidden != 1 AND contact.delflag != 1 AND contact.hidflag != 1";
	    }
	    ...
	    elseif ($this->group_id == 2) 
		{
			...
			$sql_group = ", " . TABLE_GROUPS . " AS groups
						...
						ON contact.id=address.id 
						AND contact.primaryAddress=address.refid
						WHERE contact.id=groups.id AND groups.groupid = 2 AND contact.hidden != 1 AND contact.delflag != 1 AND contact.hidflag != 1";
	    }
		...
		{ 
			$sql_group = ", " . TABLE_GROUPS . " AS groups
						...
						ON contact.id=address.id 
						AND contact.primaryAddress=address.refid
						WHERE contact.id=groups.id AND groups.groupid=$this->group_id AND contact.hidden != 1 AND contact.delflag != 1 AND contact.hidflag != 1";
		...
		switch ($this->current_letter) 
		{
			...
			case "1":	// If selecting non-alphabetical characters
				$sql_letter = " AND contact.fullname REGEXP  '^[^[:alpha:]]'";
				break;	
			default:	// If a letter is set
				$sql_letter = " AND contact.fullname LIKE '$this->current_letter%'";
				break;
		}
		...
		//Chnages for sort here
		$this->sql .= $sql_group . $sql_letter . " ORDER BY fullname $sort"  . $sql_limit;
	...
	function nav_pages($link) 
	{
		...
		{ //check whether there are multiple result pages for the request
			$this->nav_menu .= "Pages: ";
			for ($i=1; $i <= $this->total_pages; $i++) 
			{ //create an array of links to the result pages
				if ($this->current_page == $i) 
				{ //indicate the current page in the navigation 
					$this->nav_menu .= "<b>[$i]</b>\n";	
				}
				else 
				{ //create links to all other result pages
